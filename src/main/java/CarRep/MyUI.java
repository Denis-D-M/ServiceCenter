...
public class MyUI extends UI {
    ...
    protected void init(VaadinRequest vaadinRequest) {
        ...
        Label mainLabel = new Label("Сервисный центр");
        ...
        ComboBox<Master> mehComboBox = new ComboBox<>();
        mehComboBox.setCaption("Мастер");
        mehComboBox.setItems(Master.masterList);
        mehComboBox.setItemCaptionGenerator(Master::getFullName);
        ...
        ComboBox<Master> mehComboBoxUpd = new ComboBox<>();
        mehComboBoxUpd.setCaption("Мастер");
        mehComboBoxUpd.setItems(Master.masterList);
        mehComboBoxUpd.setItemCaptionGenerator(Master::getFullName);
        ...
        // ПАНЕЛЬ ДОБАВЛЕНИЯ НОВОГО МАСТЕРА

        Panel mehAddPanel = new Panel("Добавление нового мастера");
        ...
        // ПАНЕЛЬ РЕДАКТИРОВАНИЯ МАСТЕРА

        Panel mehUpdPanel = new Panel("Редактирование мастера");
        ...
        orderGrid.addColumn(Order::getMeh).setCaption("Мастер");
        ...
        Panel mehGridPanel = new Panel("Список мастеров");
        ...
        Grid<Master> mehGrid = new Grid<>("Мастера");
        mehGrid.addColumn(Master::getLastName).setCaption("Фамилия");
        mehGrid.addColumn(Master::getFirstName).setCaption("Имя");
        mehGrid.addColumn(Master::getPatronymic).setCaption("Отчество");
        mehGrid.addColumn(Master::getPayHour).setCaption("Часовая ставка");
        mehGrid.setSizeFull();
        ...
        menu.addItem("Мастера", e -> {
            workspace.removeAllComponents();
            workButtons.removeAllComponents();
            Master.masterList.clear();
            ConnectionDB.getMehList();
            mehGrid.setItems(Master.masterList);
        ...
        // КНОПКА СОХРАНЕНИЯ ДОБАВЛЕННОГО МАСТЕРА
        addMehAdd.addClickListener(e -> {
            ...
            try {
                ...
                Notification.show("Мастер не был добавлен!");
            }
            ...
            Notification.show("Мастер успешно добавлен!");
        });
        ...
        // КНОПКА СОХРАНЕНИЯ ОТРЕДАКТИРОВАННОГО МАСТЕРА
        updMeh.addClickListener(e -> {
            ...
            workButtons.removeComponent(editMeh);
            workButtons.removeComponent(deleteMeh);
            Notification.show("Мастер успешно изменён!");
        });
        ...
        deleteMeh.addClickListener(e -> {
            if (ConnectionDB.ifMeh(mehGrid.asSingleSelect().getValue().getFullName()) != 0)
                Notification.show("Мастер не может быть удалён, так как у мастера назначен заказ!");
            else {
                try {
                ...
                Notification.show("Мастер успешно удалён!");
            }
        });
        ...
        // КНОПКА ОТКРЫТИЯ ПАНЕЛИ РЕДАКТИРОВАНИЯ МАСТЕРА
        ...
        showMehOrders.addClickListener(e -> {
            if (ConnectionDB.ifMeh(mehGrid.asSingleSelect().getValue().getFullName()) == 0)
                Notification.show("У данного мастера нет заказов!");
            else {
                Notification.show("Заказов у данного мастера - " + ConnectionDB.ifMeh(mehGrid.asSingleSelect().getValue().getFullName()));
            }
        });
        ////////////////////////////////////////////////////////////////////////////////////////////
        // ВАЛИДАЦИЯ ВВОДА
